...
func _ready() -> void:
	...
	if !curTileMap.has_meta("playerInside"):
		curTileMap.set_meta("playerInside", 0)

func _on_body_entered(body: Node2D) -> void:
	if body is Player:
		player = body
		var playerInsideCheck = curTileMap.get_meta("playerInside")
		playerInsideCheck += 1
		curTileMap.set_meta("playerInside", playerInsideCheck)
